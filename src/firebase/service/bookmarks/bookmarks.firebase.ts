...
export default class BookmarkService extends SherutaDB {
	...
	static async getSingleBookmark(document_id: string): Promise<BookmarkDataDetails | null> {

		const result = await this.get({ document_id, collection_name: DBCollectionName.bookmarks }) as unknown as BookmarkDataDetails
	...
	static async getUserBookmarks(
		...
		return userBookmarks.filter((bookmark) => bookmark !== null) as BookmarkDataDetails[];
	}
