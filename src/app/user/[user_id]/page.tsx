	...
	async function getUserProfile() {
		try {
			...
			const [flatShareProfileDocs, userDoc, userInfoDocs] = await Promise.all([
				getDocs(query(collection(db, 'flatShareProfile'), where('_user_id', '==', user_id))),
				getDoc(doc(db, 'users', user_id)),
				getDocs(query(collection(db, 'userInfos'), where('_user_id', '==', user_id))),
			]);

			const formattedFlatShareProfile = flatShareProfileDocs.empty ? null : flatShareProfileDocs.docs[0].data();
			const formattedUserDoc = userDoc.exists() ? userDoc.data() : null;
			const formattedUserInfoDoc = userInfoDocs.empty ? null : userInfoDocs.docs[0].data();
